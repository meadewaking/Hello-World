...
<body class="bg-1">
	...
	<div id="wrap">
		...
		<div class="row">
			...
			<div id="content" class="col-md-12">
				...
				<form class="form-horizontal" id="frmMessages"
					action="ShowDepartmentMessageListServlet" method="post">
								...
								<div class="form-group">
									...
									<div class="col-sm-8">
										<input type="text" class="form-control" id="txtSearchTitle"
											...
											value="${searcher.title }">
									</div>
								</div>

								<div class="form-group">
									<label for="input02" class="col-sm-4 control-label">发布IP</label>
									<div class="col-sm-8">
										<input type="text" class="form-control"
											id="txtSearchPublishIP" name="txtSearchPublishIP"
											placeholder="发布IP" value="${searcher.publishIP }">
											<span class="label label-danger" id="spanSearchTitle"></span>
									</div>
								</div>

								<div class="form-group">
									<label for="input03" class="col-sm-4 control-label">内容</label>
									<div class="col-sm-8">
										<textarea class="form-control" id="txtSearchContent"
											name="txtSearchContent" placeholder="内容">${searcher.content }</textarea>
									</div>
								</div>
				...
				<section class="tile color transparent-black">
					...
					<div class="tile-header">
						<h1>
							<strong>部门消息列表</strong>
						</h1>

						<div class="controls">
							<a href="/YZY/file/messages.csv">导出数据</a> <a
								href="#" class="remove"><i class="fa fa-times"></i></a>
						</div>
					</div>
					<!-- /tile header -->
							...
							<thead>
								<tr>
									...
									<th class="sortable sort-alpha"><a
										href="javascript:sorting('MessageId');">部门编号</a></th>
									<th class="sortable sort-alpha"><a
										href="javascript:sorting('Title');">标题</a></th>
									<th class="sortable sort-alpha"><a
										href="javascript:sorting('PublishDateTime');">发布时间</a></th>
									<th class="sortable sort-alpha"><a
										href="javascript:sorting('PublishIP');">发布IP</a></th>
									<th class="sortable sort-alpha"><a
										href="javascript:sorting('PublisherId');">发布人</a></th>
									<th class="sortable sort-alpha"><a
										href="javascript:sorting('DepartmentId');">所属部门</a></th>
							...
							<tbody>
								<c:forEach items="${messages}" var="message"
									...
									<tr>
										...
										<td>${message.title}</td>
										<td>${message.publishDateTime}</td>
										<td>${message.publishIP}</td>
										<td>${message.publisher.loginName}</td>
										<td>${message.department.departmentName}</td>
	...
	<script type="text/javascript">
		/*	$("#frmLoginLogs").submit(function() {
				var flag = true;
				if ($("#txtLoginDateTime").val().length == 0) {
					$("#spanLoginDateTime").text("登录时间不能为空!");
					flag = false;
				} else
					$("#spanLoginDateTime").text("");
				return flag;
			}); */
		...
		$("#btnSearch").click(
				function() {
					var flag = true;
					if ($("#txtSearchTitle").val().length == 0
							&& $("#txtSearchPublishIP").val().length == 0
							&& $("#txtSearchContent").val().length == 0) {
						$("#spanSearchTitle").text("查询条件不能为空！");
						flag = false;
					} else
						$("#spanSearchTitle").text("");
