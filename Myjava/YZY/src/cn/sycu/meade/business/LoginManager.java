...
public class LoginManager {
	...
	public LoginBean searchEmail(String Email) {
		try {
	...
	public LoginBean isLoginByEmail(String loginName, String email) {
		LoginBean login = null;
		try {
			login = dao.selectByLoginName(loginName);
			if (login != null && login.getEmail().compareToIgnoreCase(email) == 0)
				return login;
		} catch (SQLException e) {
			throw new RuntimeException("验证登录信息时有错，错误原因：" + e.getMessage());
	...
	public LoginBean isLoginByPassword(String loginName, String password) {
		LoginBean login = null;
		try {
			login = dao.selectByLoginName(loginName);
			if (login != null && login.getPassword().compareTo(password) == 0)
	...
	public boolean resetPassword(String loginName, String newPassword) {
		...
		try {
			...
			if (login != null) {
				newPassword = MD5Util.md5Encode(newPassword);// 明文密码加密后比较
				login.setPassword(newPassword);
				if (dao.update(login) > 0)
					return true;
				else
				...
				throw new RuntimeException("登录信息不存在，不能修改密码！");
			}
		} catch (SQLException e) {
			throw new RuntimeException("重置密码时出错，错误原因：" + e.getMessage());
		} catch (Exception e) {
			throw new RuntimeException("重置密码时密码加密处理出错，错误原因：" + e.getMessage());
		}
	}
	...
	public boolean existedLoginName(String loginName) {
		return search(loginName) != null;
	}

	public boolean existedEmail(String Email) {
		return searchEmail(Email) != null;
	}
