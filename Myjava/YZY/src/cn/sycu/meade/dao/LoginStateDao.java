...
public class LoginStateDao {
	...
	public ArrayList<LoginStateBean> select(LoginStateSearcher searcher, String sortString) throws SQLException {
		...
		try {
			...
			try {
				fos = new FileOutputStream("D:/Program Files (x86)/Apache Software Foundation/Tomcat 8.0/webapps/YZY/file/loginStates.csv"); // 打开保存文件
				osw = new OutputStreamWriter(fos,"GB2312");
				osw.write("登录状态编号,登录状态名称,登录状态,描述\r\n");
				for (int i = 0; i < loginStates.size(); i++) {
					osw.write(String.valueOf(loginStates.get(i).getLoginStateId()));
					osw.write(",");
					osw.write(loginStates.get(i).getLoginStateName().length() > 0 ? loginStates.get(i).getLoginStateName() + "," : "NULL,");
					osw.write(loginStates.get(i).getCanLogin().toString() + ",");
					osw.write(loginStates.get(i).getDescription().length() > 0 ? loginStates.get(i).getDescription() + "\r\n" : "NULL\r\n");
				}
